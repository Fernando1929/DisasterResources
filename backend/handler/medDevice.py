...
class MedDeviceHandler:
    def build_mdevice_dict(self, row):
        ...
        result['mdevice_name'] = row[2]
        result['mdevice_brand'] = row[3]
        result['mdevice_quantity'] = row[4]
        result['mdevice_price'] = row[5]
        result['mdevice_type'] = row[6]
        result['mdevice_model'] = row[7]
        result['mdevice_condition'] = row[8]
        result['mdevice_power_type'] = row[9]
        return result

    def build_mdevice_attributes(self, mdevice_id, supplier_id, mdevice_name, mdevice_brand, mdevice_quantity, mdevice_price, mdevice_type, mdevice_model, mdevice_condition, mdevice_power_type):
        result = {}
    ...
    def insertMedDevice(self, json):
        ...
        if supplier_id and mdevice_name and mdevice_brand and mdevice_quantity and mdevice_price and mdevice_type and mdevice_model and mdevice_condition and mdevice_power_type:
            ...
            result = self.build_mdevice_attributes(resource_id, supplier_id, mdevice_name, mdevice_brand, mdevice_quantity, mdevice_price, mdevice_type, mdevice_model, mdevice_condition, mdevice_power_type)
            return jsonify(Medical_Device = result), 201
        else:
            return jsonify(Error = "Unexpected attributes in post request"), 400

    def updateMedDevice(self, mdevice_id, json):
        hequip_dao = MedDeviceDAO()
        if not hequip_dao.getMedDeviceById(mdevice_id):
            return jsonify(Error = "Medical Device not found."), 404
        else:
            supplier_id = json["supplier_id"]
            mdevice_name = json["mdevice_name"]
            ...
            mdevice_power_type = json["mdevice_power_type"]
            if supplier_id and mdevice_name and mdevice_brand and mdevice_quantity and mdevice_price and mdevice_type and mdevice_model and mdevice_condition and mdevice_power_type:
                resource_id = hequip_dao.update(mdevice_id, mdevice_type, mdevice_model, mdevice_condition, mdevice_power_type)
                resource_dao = ResourceDAO()
